...
# --- guru_id 정규화 (이전 버전 기능 통합) -------------------------

def _normalize_guru_id(guru_id: Optional[str]) -> str:
    """
    프론트/클라이언트에서 들어오는 다양한 멘토 별칭을
    내부 표준 ID로 정규화한다.

    예)
    - "warren", "warren_buffett"  → "buffett"
    - "wood", "cathie", "cathie_wood" → "ark"
    - "lynch", "peter", "peter_lynch" → "lynch"
    """
    if not guru_id:
        return "buffett"

    key = guru_id.strip().lower()
    mapping = {
        # 워렌 버핏
        "buffett": "buffett",
        "warren": "buffett",
...
async def get_initial_message(guru_id: str) -> Dict[str, object]:
    ...
    canonical_guru = _normalize_guru_id(guru_id)

    intent = "news_analysis"
    audience = ""
    persona = _load_persona(canonical_guru, intent, audience)
    system_prompt = _compose_system_prompt(canonical_guru, intent, audience, persona, None, "")

    # 버핏의 경우 고정된 인사 메시지 사용 (원래 문장으로 복원)
    if canonical_guru in ["buffett", "warren_buffett"]:
...
async def generate_response(
    ...
    session_id, state = get_or_create_session(session_id, canonical_guru)
    intent = _infer_intent(user_input)
    audience = _infer_audience(user_input)
    symbols = find_symbols(user_input)

    logger.debug("intent=%s audience=%s symbols=%s guru=%s", intent, audience, symbols, canonical_guru)

    persona = _load_persona(canonical_guru, intent, audience)
    
    # symbols가 있으면 해당 심볼의 컨텍스트를 로드
    cached_context = _load_cached_context(canonical_guru, symbols)
    ...
    if not cached_context:
        try:
            ...
            cached_context = load_profile_summary(canonical_guru)
            if cached_context:
                section = cached_context.get("section", "unknown")
                logger.info(
                    "Loaded 13F profile summary for guru: %s (section: %s, updated: %s)",
                    canonical_guru,
                    section,
                    cached_context.get("updated") or cached_context.get("period_end") or "unknown",
                )
                logger.debug(
                    "13F summary content preview: %s",
                    (cached_context.get("content") or cached_context.get("text") or "")[:200],
                )
            else:
                # 13F가 없으면 일반 최신 요약 로드
                cached_context = load_latest_summary(canonical_guru, symbols=None)
                if cached_context:
                    logger.info("Loaded latest summary (non-13F) for guru: %s", canonical_guru)
                else:
                    logger.warning("No summary found for guru: %s", canonical_guru)
        except Exception as exc:
            logger.error("Failed to load 13F summary for guru %s: %s", canonical_guru, exc, exc_info=True)
    ...
    # 비기너(new_user)는 답변 본문을 2-3줄 이하로 제한
    if audience == "new_user":
