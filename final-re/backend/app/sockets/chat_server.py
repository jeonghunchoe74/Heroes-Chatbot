...
async def _request_chatbot_response(message: str, guru: str, room: str = ROOM) -> str:
    """
    LangGraph 기반 챗봇 응답 생성 (직접 호출)
    
    HTTP 호출 대신 unified_chatbot_service를 직접 호출합니다.
    """
    ...
    try:
        ...
        response, _ = await generate_response_unified(
        ...
        logger.error(f"Chatbot response failed: {exc}", exc_info=True)
        return f"죄송합니다. 응답 생성 중 오류가 발생했습니다: {exc}"
