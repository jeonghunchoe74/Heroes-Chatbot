...
def get_guru_prompt(
    ...
    # [DEPRECATED] 레거시 prompts/summaries 폴더는 더 이상 사용하지 않음
    # 새로운 RAG 구조 사용
    persona_chunks = []
    latest_summary = None
    ...
    try:
        from app.services.rag_service import (
            ...
            get_portfolio_history_snippets,
        ...
        intent_map = {
            ...
            "company_query": Intent.STOCK_ANALYSIS,
            "greeting": Intent.SMALLTALK,
        }
        intent_enum = intent_map.get(intent, Intent.SMALLTALK)
        
        # 새로운 RAG 구조에서 철학 스니펫 가져오기
        philosophy_snippets = get_guru_philosophy_snippets(normalized_id, intent=intent_enum, limit=5)
        persona_chunks = [{"text": s, "content": s, "section": "philosophy"} for s in philosophy_snippets]
        
        # 포트폴리오 히스토리 가져오기
        portfolio_snippets = get_portfolio_history_snippets(normalized_id, limit=1)
        if portfolio_snippets:
            latest_summary = {
                "content": portfolio_snippets[0],
                "text": portfolio_snippets[0],
                "section": "portfolio_history",
            }
    except Exception as exc:
        logger.debug(f"Failed to load from new RAG structure: {exc}")
    
    picked_chunks = persona_chunks  # 이미 필터링됨
