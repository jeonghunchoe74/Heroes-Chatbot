...
def _format_us_period_label(as_of_text: Optional[str]) -> str:
    """YYYY-MM-DD 문자열을 Quarter 기반 라벨로 변환 (예: 2018/Q3)."""
    if not as_of_text:
        return "N/A"
    normalized = as_of_text.split("T")[0]
    try:
...
US_MACRO_SUMMARY_LABELS = {
    "CPI_INDEX": "CPI 지수",
    "UNEMPLOYMENT_RATE": "실업률",
    "DGS10_YIELD": "10년물 국채",
    "TIPS10_YIELD": "10년물 TIPS",
    "PCE_SERIES": "PCE",
    "GDP_REAL": "실질 GDP",
    "VIX_LEVEL": "VIX",
    "M2_INDEX": "M2",
...
def _build_us_macro_summary(period: str, macro_values: Dict[str, Any]) -> str:
    summary_parts: List[str] = []
    for key, label in US_MACRO_SUMMARY_LABELS.items():
        if key in macro_values:
            summary_parts.append(f"{label} {_format_macro_value(macro_values[key])}")
    if not summary_parts:
        return ""
    return f"{period} 미국 경제지표: " + ", ".join(summary_parts)


def _extract_us_macro_from_portfolio_rows(
    rows: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """guru_portfolio_with_macro.jsonl에서 미국 매크로 지표만 추출."""
    period_map: Dict[str, Dict[str, Any]] = {}

    for row in rows:
        metadata = row.get("metadata") or {}
        macro_values = metadata.get("macro")
        if not isinstance(macro_values, dict):
            continue

        as_of = row.get("as_of") or row.get("period_end") or row.get("period_start")
        period = _format_us_period_label(as_of)
        ...
        summary = _build_us_macro_summary(period, indicators)

        metadata_copy = dict(metadata)
        metadata_copy["source"] = metadata.get("source") or "guru_portfolio_with_macro.jsonl"
        metadata_copy["indicators"] = indicators
        metadata_copy["as_of_date"] = as_of
        guru_id = row.get("guru_id")
        if guru_id:
            existing_ids = metadata_copy.get("portfolio_guru_ids")
            if isinstance(existing_ids, list):
                if guru_id not in existing_ids:
                    existing_ids.append(guru_id)
            else:
                metadata_copy["portfolio_guru_ids"] = [guru_id]

        # 동일 분기의 중복은 가장 최신 값으로 유지
        record = period_map.get(period)
        if record:
            meta = record.setdefault("metadata", {})
            ids = meta.setdefault("portfolio_guru_ids", [])
            if guru_id and guru_id not in ids:
                ids.append(guru_id)
            continue
        ...
        period_map[period] = {
            ...
            "metadata": {
                **metadata_copy,
                "portfolio_snapshot_guru_id": guru_id,
            },
        }

    sorted_records = sorted(
        period_map.values(),
        key=lambda x: (x.get("metadata", {}).get("as_of_date") or x["period"]),
        reverse=True,
    )
...
def initialize_rag_cache() -> None:
    ...
    # 2. Portfolio history loading
    all_history_rows: List[Dict[str, Any]] = []
    if PORTFOLIO_MACRO_FILE.exists():
        all_history_rows = _read_jsonl_safe(PORTFOLIO_MACRO_FILE)
        ...
        if len(_portfolio_cache) == 0 and len(all_history_rows) > 0:
            # 서버가 첫번째 row의 구조 확인
            logger.warning(
                f"[RAG_SERVICE] No portfolio rows found. "
                f"First row keys: {list(all_history_rows[0].keys())}, "
                f"doc_type: {all_history_rows[0].get('doc_type')}, "
                f"section: {all_history_rows[0].get('section')}"
            )
    else:
    ...
    # 4. Macro data (US) - guru_portfolio + JSONL + CSV merge
    us_rows: List[Dict[str, Any]] = []
    if all_history_rows:
        us_rows = _extract_us_macro_from_portfolio_rows(all_history_rows)
        if us_rows:
            logger.info(
                "[RAG_SERVICE] Loaded %s US macro rows from %s",
                len(us_rows),
                PORTFOLIO_MACRO_FILE.name,
            )
        else:
            logger.warning(
                f"[RAG_SERVICE] No US macro indicators embedded in {PORTFOLIO_MACRO_FILE.name}"
            )
    else:
        logger.debug("[RAG_SERVICE] Portfolio history unavailable; skipping US macro extraction")
    ...
    if not us_rows:
        logger.warning(
            "[RAG_SERVICE] No US macro data available from portfolio history or CSV file"
        )
...
def get_macro_regime(
    region: str = "KR",
    last_n_quarters: Optional[int] = 4,
) -> List[Dict[str, Any]]:
    """
    ...
    if last_n_quarters is None or last_n_quarters <= 0:
        result = filtered
    else:
...
# 하위 호환성을 위한 별칭
def get_macro_regime_snippets(region: str, last_n: Optional[int] = 4) -> List[Dict[str, Any]]:
