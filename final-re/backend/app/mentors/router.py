...
def route_query(
    ...
    # 분석 키워드를 포함한 경우(특정 예외 제외) 회사 분석으로 유도
    analysis_keywords = ['분석', '어때', '어떤가', '해석', '평가', '전망']
    if symbols and _contains(analysis_keywords) and intent not in (
        Intent.HISTORICAL_DATA,
        Intent.MACRO_OUTLOOK,
        Intent.PHILOSOPHY,
    ):
        intent = Intent.COMPANY_ANALYSIS

    # 지표 키워드만 있을 경우 메트릭스로 전환
