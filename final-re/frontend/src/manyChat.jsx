...
export default function App() {
  ...
  useEffect(() => {
    ...
    const onGuru = (payload = {}) => {
      const guruId = payload.guruId || payload.id;
      if (guruId) {
        console.log("[onGuru] Received guru change:", guruId, payload);
        setCurrentGuru(guruId);
      }
    };
  ...
  // selectedGuru가 변경될 때 currentGuru도 함께 업데이트 (서버 이벤트가 없을 때만)
  useEffect(() => {
    // 서버에서 받은 이벤트가 우선이므로, 초기 로드 시에만 동기화
    if (!joined) {
      setCurrentGuru(selectedGuru);
    }
  }, [selectedGuru, joined]);
  ...
  const guruLabels = {
    lynch: "피터 린치",
    buffett: "워렌 버핏",
    wood: "캐시 우드", // 서버에서 wood로 오는 경우도 처리
  };

  const guruFaces = {
    lynch: peterface,
    buffett: buffettface,
    wood: woodface, // 서버에서 wood로 오는 경우도 처리
  };
  ...
  const guruSendButtons = {
    buffett: send2,
    wood: send3, // 서버에서 wood로 오는 경우도 처리
    lynch: send1,
  };
